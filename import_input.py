    ...
    def __init__(self, input_settings):
        ...
        for i in range(len(self.input_names)):
            with open(self.input_names[i]) as input_i:
                ...
                '''
                stress_eff_pos = inp["Mean_Value_of_POSITIVE_EFFECTIVE_STRESS_VECTOR"]["Segment_1"]
                stress_eff_neg = inp["Mean_Value_of_NEGATIVE_EFFECTIVE_STRESS_VECTOR"]["Segment_1"]
                sig_eff_pos_le += stress_eff_pos[0:dam_start_step[i]]
                sig_eff_pos_nl += stress_eff_pos[dam_start_step[i]:last_step_conv[i]]
                sig_eff_neg_le += stress_eff_neg[0:dam_start_step[i]]
                sig_eff_neg_nl += stress_eff_neg[dam_start_step[i]:last_step_conv[i]]
                '''
        ...
        '''
        sig_eff_pos_le = np.asarray(sig_eff_pos_le)
        sig_eff_pos_nl = np.asarray(sig_eff_pos_nl)
        sig_eff_neg_le = np.asarray(sig_eff_neg_le)
        sig_eff_neg_nl = np.asarray(sig_eff_neg_nl)
        '''
        ...
        '''
        self.GetPositiveEffectiveStressLinear    = sig_eff_pos_le * self.resize_fac
        self.GetPositiveEffectiveStressNonlinear = sig_eff_pos_nl * self.resize_fac
        self.GetNegativeEffectiveStressLinear    = sig_eff_neg_le * self.resize_fac
        self.GetNegativeEffectiveStressNonlinear = sig_eff_neg_nl * self.resize_fac
        '''
